...
Creates a structured archive in Supabase Storage with markdown previews
and original files for download, enabling source citations with clickable links.
"""
...
class ArchiveGenerator:
    ...
    Creates a folder structure in Supabase Storage for each email that enables:
    ...
    async def generate_archive(
        ...
        self.storage.upload_file(
            f"{folder_id}/email.eml",
            eml_content,
            "message/rfc822",
        )

        # Process attachments to upload originals and create .md previews
        ...
        # Generate email markdown (without [View] links initially for attachments without content)
        email_md_content = self._generate_email_markdown(
            parsed_email, attachment_files, folder_id
        )
        self.storage.upload_text(
            f"{folder_id}/email.eml.md",
            email_md_content,
        )
    ...
    def _process_attachments_to_files(
        ...
        """Process all attachments, uploading originals and markdown previews."""
        ...
        for att in parsed_email.attachments:
            ...
            if not skipped:
                ...
                if parsed_content:
                    md_content = self._generate_content_markdown(
                        filename,
                        content_type,
                        att.size_bytes,
                        parsed_content,
    ...
    def _generate_content_markdown(
        self,
        filename: str,
        content_type: str,
        size_bytes: int,
    ...
    def update_attachment_markdown(
        ...
        if self._should_skip_markdown(filename):
            logger.debug("  Skipping - file is already markdown")
            return None

        if not parsed_content or not parsed_content.strip():
            logger.debug(
                f"  Skipping - no parsed_content (len={len(parsed_content) if parsed_content else 0})"
            )
            return None
