-- Migration 004c: Fix topic_ids filter operator
-- Two issues fixed:
-- 1. The ?| operator doesn't properly match JSONB array elements with text array values.
--    This fix uses jsonb_array_elements_text to properly compare stored UUIDs with filter values.
-- 2. pgvector query planner issue: ORDER BY embedding <=> $1 combined with EXISTS subqueries
--    returns no results. Fixed by computing similarity in subquery and ordering by that column.
...
BEGIN
    ...
    -- Use subquery to avoid pgvector query planner issue with ORDER BY <=> and EXISTS subqueries
    -- Computing similarity in inner query and ordering by that column works correctly
    RETURN QUERY
    SELECT * FROM (
        SELECT
            c.chunk_id,
        ...
        WHERE c.embedding IS NOT NULL
          ...
          -- topic_ids uses OR logic: match if ANY stored topic_id equals ANY filter value
          -- Uses jsonb_array_elements_text to properly extract and compare string values
...
Uses jsonb_array_elements_text for proper UUID string comparison.
Uses subquery approach to avoid pgvector ORDER BY planner issue with EXISTS subqueries.';
