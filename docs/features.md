...
Arguments:
  FILE_PATHS          One or more EML paths to mark FAILED

Options:
  -o, --output-dir PATH   Output directory (default: data/output)
  -r, --reason TEXT       Error reason recorded on the entry (default: manual_mark_failed)
```

**When to use:**
- `MTSS validate` surfaces data-integrity issues tied to a handful of specific emails
- You want to re-process those emails only, without wiping the whole run
...
# Mark several with a descriptive reason
MTSS mark-failed data/emails/a.eml data/emails/b.eml \
  --reason "zip_context_missing"

# Non-default output directory
MTSS mark-failed ./some.eml --output-dir ./alt-output

# Then re-ingest just the flagged emails
MTSS ingest --retry-failed
```

### `MTSS clean-archive-md`

Rewrite every `.md` file under `data/output/archive/` using the current LlamaParse image-ref stripping regex. Zero API cost, idempotent, and safe to re-run.

```bash
MTSS clean-archive-md [OPTIONS]

Options:
  -o, --output-dir PATH   Output directory (default: data/output)
  -n, --dry-run           List files that would change without writing
```

**When to use:**
- After updating the image-ref stripping logic (e.g. to handle a new malformed LlamaParse pattern)
- To retroactively clean archived markdown that was generated by an older pipeline version
- `MTSS validate` reports "broken markdown links" flagged as "truncated by parens"

**What it strips (alt-text is preserved):**
- `<img src="..." alt="diagram" />` → `diagram`
- `![logo](page_1_image_1_v2.jpg)` → `logo`
- `![photo](image)` → `photo`

**What it touches:** human-readable markdown under `archive/` only. Documents, chunks, and embeddings are left alone.

**Examples:**
```bash
# Preview
MTSS clean-archive-md --dry-run

# Apply
MTSS clean-archive-md

# Against a non-default output dir
